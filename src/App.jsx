...
const Factory = ({ brands }) => {
  ...
  const go = async () => {
    ...
    try { const r = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2500,system:sys,messages:[{role:"user",content:msg}]})}); const d = await r.json(); const raw = d.content?.map(c=>c.text||"").join("")||""; if(fmt==="text"){setTxt(raw);setResult({t:"text"});}else{try{const pd=JSON.parse(raw.replace(/```json|```/g,"").trim());const imgUrl=pd.image_prompt?"https://image.pollinations.ai/prompt/"+encodeURIComponent(pd.image_prompt.split(",").slice(0,3).join(","))+"?width=1024&height=1024&nologo=true&seed="+Date.now():null;setResult({t:fmt,d:pd,img:imgUrl});}catch{setTxt(raw);setResult({t:"text"});}} } catch{setTxt("Error.");setResult({t:"text"});} setLoading(false);
  };
