...
const Factory = ({ brands, gemKey }) => {
  ...
  const go = async () => {
    ...
    try { const r = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2500,system:sys,messages:[{role:"user",content:msg}]})}); const d = await r.json(); const raw = d.content?.map(c=>c.text||"").join("")||""; if(fmt==="text"){setTxt(raw);setResult({t:"text"});}else{try{const pd=JSON.parse(raw.replace(/```json|```/g,"").trim());setResult({t:fmt,d:pd});console.log("GEMKEY:",gemKey?"YES":"NO");console.log("IMAGE_PROMPT:",pd.image_prompt);if(gemKey&&pd.image_prompt){try{const gr=await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-image-generation:generateContent?key="+gemKey,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:"Generate professional photo: "+pd.image_prompt+". NO text or watermarks."}]}],generationConfig:{responseModalities:["IMAGE","TEXT"]}})});const gd=await gr.json();for(const p of(gd.candidates?.[0]?.content?.parts||[]))if(p.inlineData){setResult(r=>({...r,img:"data:"+p.inlineData.mimeType+";base64,"+p.inlineData.data}));break;}}catch(e){console.log("Gemini:",e)}}}catch{setTxt(raw);setResult({t:"text"});}} } catch{setTxt("Error.");setResult({t:"text"});} setLoading(false);
  };
