...
const Factory = ({ brands, gemKey }) => {
  ...
  const go = async () => {
    ...
    const sys = "Eres DIRECTOR CREATIVO SENIOR agencia Bogotá. MARCA: "+brand.name+" | "+brand.industry+" | TONO: "+(brand.tone||"Profesional")+" | AUDIENCIA: "+(brand.audience||"General")+" | VOZ: "+(brand.brandVoice||"Profesional")+" | PRODUCTOS: "+(brand.products||"N/A")+". REGLAS: 1)Gancho 2)3-5 emojis 3)Gancho-Valor-CTA 4)8 hashtags 5)Español colombiano tú 6)Valor primero. SOLO JSON sin markdown.";
    const fmt = ct.fmt;
    let msg = fmt==="visual"?"Post: "+topic+". JSON:{headline:max 8 pal,subtext:sub,caption:3-5 lineas emojis,hashtags:8 hashtags,image_prompt:english professional photo prompt for this topic no text}":fmt==="carousel"?"Carrusel 5 slides: "+topic+". JSON:{slides:[{title:..,body:emojis,emoji:..}],caption:CTA,hashtags:8}":fmt==="reel"?"Reel 5 escenas: "+topic+". JSON:{scenes:[{title:..,duration:3s,emoji:..,visual:camara,text_overlay:texto+emoji,audio:musica,transition:tipo}],caption:CTA,hashtags:8}":ct.label+": "+topic+". Emojis, Gancho-Valor-CTA. 8 hashtags.";
    try {
    ...
    } catch{setTxt("Error de conexión.");setResult({t:"text"});}
    setLoading(false);
  };
...
} 
 {
  ...
  const go = async () => {
    ...
    const fmt = ct.fmt;
    let msg = fmt==="visual"?`Post:"${topic}".JSON:{"headline":"8pal","subtext":"sub","caption":"3-5 líneas emojis","hashtags":"8","image_prompt":"english: professional photo for this topic, commercial photography, no text"}`:fmt==="carousel"?`Carrusel 5:"${topic}".JSON:{"slides":[{"title":"..","body":"emojis","emoji":".."}],"caption":"CTA","hashtags":"8"}`:fmt==="reel"?`Reel 5 escenas:"${topic}".JSON:{"scenes":[{"title":"..","duration":"3s","emoji":"..","visual":"cámara","text_overlay":"texto+emoji","audio":"música","transition":"tipo"}],"caption":"CTA","hashtags":"8"}`:`${ct.label}:"${topic}".Emojis,Gancho→Valor→CTA.8 hashtags.`;
    try { const r = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2500,system:sys,messages:[{role:"user",content:msg}]})}); const d = await r.json(); const raw = d.content?.map(c=>c.text||"").join("")||""; if(fmt==="text"){setTxt(raw);setResult({t:"text"});}else{try{const pd=JSON.parse(raw.replace(/```json|```/g,"").trim());const hasImg=pd.image_prompt;setResult({t:fmt,d:pd,img:hasImg?"https://image.pollinations.ai/prompt/"+encodeURIComponent(pd.image_prompt)+"?width=1024&height=1024&nologo=true&seed="+Date.now():null});if(pd.image_prompt){setResult(r=>({...r,img:"https://image.pollinations.ai/prompt/"+encodeURIComponent(pd.image_prompt)+"?width=1024&height=1024&nologo=true"}))};console.log("GEMKEY:",gemKey?"YES":"NO");console.log("IMAGE_PROMPT:",pd.image_prompt);if(gemKey&&pd.image_prompt){try{const gd=await gr.json();for(const p of(gd.candidates?.[0]?.content?.parts||[]))if(p.inlineData){setResult(r=>({...r,img:"data:"+p.inlineData.mimeType+";base64,"+p.inlineData.data}));break;}}catch(e){console.log("Gemini:",e)}}}catch{setTxt(raw);setResult({t:"text"});}} } catch{setTxt("Error.");setResult({t:"text"});} setLoading(false);
  };
  if(!brands.length) return <Section title="Crear Contenido"><Card style={{textAlign:"center",padding:48}}><div style={{fontSize:48,marginBottom:12}}>🏢</div><div style={{fontSize:16,fontWeight:600,color:t.tx}}>Primero crea una marca en "Mis Marcas"</div></Card></Section>;
  return (
    <Section title="Crear Contenido" sub="Genera contenido profesional con IA." right={<Badge>Claude AI</Badge>}>
      ...
      {loading&&<Card style={{padding:48,textAlign:"center"}}><div style={{width:48,height:48,border:`3px solid ${t.brd}`,borderTop:`3px solid ${brand?.color||t.ac}`,borderRadius:"50%",animation:"spin .8s linear infinite",margin:"0 auto 16px"}}/><div style={{color:t.tx,fontSize:16,fontWeight:600}}>Generando para {brand?.name}...</div></Card>}
      {result&&!loading&&result.t==="text"&&<Card><div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}><span style={{fontSize:13,fontWeight:600,color:t.txS}}>{brand?.emoji} {brand?.name}</span><CopyBtn text={txt}/></div><div style={{fontSize:14,color:t.tx,lineHeight:1.8,whiteSpace:"pre-wrap"}}>{txt}</div></Card>}
      {result&&!loading&&result.t==="visual"&&result.d&&<Card>{result.img&&<div style={{marginBottom:16,borderRadius:12,overflow:"hidden"}}><img src={result.img} alt="" style={{width:"100%",maxHeight:420,objectFit:"cover",borderRadius:12}}/></div>}{result.img&&<div style={{marginBottom:16,borderRadius:12,overflow:"hidden"}}><img src={result.img} alt="" style={{width:"100%",maxHeight:400,objectFit:"cover"}}/></div>}<div style={{fontSize:22,fontWeight:800,color:t.tx,marginBottom:6}}>{result.d.headline}</div>{result.d.subtext&&<div style={{color:t.txS,marginBottom:12}}>{result.d.subtext}</div>}<div style={{fontSize:14,color:t.tx,lineHeight:1.7,whiteSpace:"pre-wrap"}}>{result.d.caption}</div>{result.d.hashtags&&<div style={{fontSize:12,color:brand?.color,marginTop:10}}>{result.d.hashtags}</div>}<div style={{marginTop:12}}><CopyBtn text={`${result.d.caption}\n\n${result.d.hashtags||""}`} label="📱 Copiar"/></div></Card>}
      {result&&!loading&&result.t==="carousel"&&result.d?.slides&&<Card>{result.d.slides.map((sl,i)=><div key={i} style={{padding:"12px 0",borderBottom:i<result.d.slides.length-1?`1px solid ${t.brd}`:"none"}}><div style={{fontSize:14,fontWeight:600,color:t.tx}}>{sl.emoji} Slide {i+1}: {sl.title}</div><div style={{fontSize:13,color:t.txS,marginTop:3}}>{sl.body}</div></div>)}{result.d.caption&&<div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${t.brd}`,fontSize:14,color:t.tx,whiteSpace:"pre-wrap"}}>{result.d.caption}</div>}<div style={{marginTop:12}}><CopyBtn text={result.d.slides.map((s,i)=>`${s.emoji} Slide ${i+1}: ${s.title}\n${s.body}`).join("\n\n")+`\n\n${result.d.caption||""}\n${result.d.hashtags||""}`} label="📋 Todo"/></div></Card>}
      {result&&!loading&&result.t==="reel"&&result.d?.scenes&&<Card>{result.d.scenes.map((sc,i)=><div key={i} style={{padding:"14px 0",borderBottom:i<result.d.scenes.length-1?`1px solid ${t.brd}`:"none"}}><div style={{fontSize:15,fontWeight:700,color:t.tx,marginBottom:4}}>🎬 Escena {i+1}: {sc.title} <span style={{fontSize:12,color:t.txM}}>({sc.duration})</span></div><div style={{fontSize:13,color:t.txS,marginBottom:4}}>📹 {sc.visual}</div><div style={{fontSize:14,fontWeight:700,color:brand?.color}}>📝 {sc.text_overlay}</div>{sc.audio&&<div style={{fontSize:12,color:t.txM,marginTop:4}}>🎵 {sc.audio}</div>}</div>)}{result.d.caption&&<div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${t.brd}`,fontSize:14,color:t.tx,whiteSpace:"pre-wrap"}}>{result.d.caption}</div>}<div style={{marginTop:12}}><CopyBtn text={result.d.scenes.map((s,i)=>`🎬 ${i+1}(${s.duration}):${s.title}\n📹${s.visual}\n📝${s.text_overlay}\n🎵${s.audio||""}`).join("\n\n")+`\n\n${result.d.caption||""}\n${result.d.hashtags||""}`} label="📋 Storyboard"/></div></Card>}
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </Section>
